...
<section><span class="section-no">03</span><h2>趋势判断</h2><ol class="editorial-list"><li v-for="x in sectionItems(report.sections.trends)" :key="x">{{x}}</li></ol></section>
<div class="two-sections"><section><span class="section-no">04</span><h2>潜在机会</h2><ul><li v-for="x in sectionItems(report.sections.opportunities)" :key="x">{{x}}</li></ul></section><section><span class="section-no">05</span><h2>风险与不确定性</h2><ul><li v-for="x in sectionItems(report.sections.risks)" :key="x">{{x}}</li></ul></section></div>
<section class="action-section"><span class="section-no">06</span><h2>建议下一步</h2><div v-for="(x,i) in sectionItems(report.sections.actions)" :key="x" class="action"><b>{{String(i+1).padStart(2,"0")}}</b><p>{{x}}</p></div></section>
<section><span class="section-no">07</span><h2>信息来源</h2><a class="source" v-for="s in report.sources" :key="s.url" :href="s.url" target="_blank" rel="noreferrer"><div><b>{{s.title}}</b><small>{{sourceDate(s.publishedAt)}}</small></div><i>↗</i></a><p v-if="!report.sources.length" class="muted">本次未获取到可展示的外部来源，报告使用演示内容生成。</p></section></article>
